...
import { getSBChapterSloks } from "../../../services/sbService";
import type { SBSlok } from "../../../types/sb";
import PageContainer from "../../../components/layout/PageContainer";

export default function SBChapterPage() {
  const { cantoNumber, chapterNumber } = useParams();

  const [sloks, setSloks] = useState<SBSlok[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!cantoNumber || !chapterNumber) return;

    getSBChapterSloks(cantoNumber, chapterNumber)
      .then(setSloks)
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [cantoNumber, chapterNumber]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-50 via-sky-50 to-white text-slate-900">
      <PageContainer>
        <div className="mx-4">
          <Link
            to={`/books/srimad-bhagavatam/canto/${cantoNumber}`}
            className="text-sm font-semibold text-blue-700 hover:underline"
          >
            ← Back to Canto {cantoNumber}
          </Link>
        </div>
          ...
          {loading ? (
            ...
            <div className="grid gap-5">
              {sloks.map((slok) => (
                <Link
                  key={slok.id}
                  to={`/books/srimad-bhagavatam/canto/${slok.canto_number}/chapter/${slok.chapter_number}/verse/${slok.verse_key}`}
